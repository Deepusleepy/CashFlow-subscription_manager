  ...
  return <main className="app-shell">
    ...
    <section className="content">
      <header className="topbar">
        ...
        <div className="header-actions"><label className="month-select"><CalendarDays size={16}/><select value={selectedMonthKey} aria-label="Select month" onChange={event => setActiveMonth(event.target.value)}>{availableMonths.slice().reverse().map(month => <option key={month} value={month}>{new Intl.DateTimeFormat("en-IN", { month: "long", year: "numeric" }).format(new Date(`${month}-01T00:00:00`))}</option>)}</select></label><button className="entry-btn" onClick={() => setEntryOpen(true)}><Plus size={16}/> Add subscription</button><Link href="/login" className={`data-status ${dataSource === "live" ? "live" : ""}`}><span className="status-dot"/>{dataSource === "live" ? "Live data" : sessionEmail ? "Connected" : "Login"}</Link><button className="icon-btn" aria-label={`Notifications, ${alerts.length} unread`} title={`${alerts.length} subscription${alerts.length === 1 ? "" : "s"} to review`} onClick={() => alerts[0] ? setSelected(alerts[0]) : setView("subscriptions")}><Bell size={18}/><i/></button><button onClick={runScan} disabled={scanning} className="scan-btn"><ScanLine size={17}/>{scanning ? "Scanning…" : "Run AI Scan"}</button></div>
      </header>
...
function Dashboard({ dataset, currentMonthLabel, selectedMonthKey, subscriptions, anomalies, monthlySpend, potentialSavings, learningCount, alertsCount, chartsReady, showAllRecent, onToggleShowAll, onSelect, onNavigate }: { dataset: Transaction[]; currentMonthLabel: string; selectedMonthKey: string; subscriptions: Subscription[]; anomalies: Anomaly[]; monthlySpend: number; potentialSavings: number; learningCount: number; alertsCount: number; chartsReady: boolean; showAllRecent: boolean; onToggleShowAll: () => void; onSelect: (item: Subscription | Anomaly) => void; onNavigate: () => void }) {
  ...
  return <div className="dashboard-page">
    ...
    <section className="dashboard-grid"><div className="panel trend-panel"><div className="panel-head"><div><p className="eyebrow">SPENDING RHYTHM</p><h2>Monthly outflow</h2></div>{previousMonthKey && <Tone tone={spendDelta <= 0 ? "good" : "risk"}>{spendDelta < 0 ? "↓" : "↑"} {currency(Math.abs(spendDelta))} vs {new Intl.DateTimeFormat("en-IN", { month: "short" }).format(new Date(`${previousMonthKey}-01T00:00:00`))}</Tone>}</div><div className="chart">{chartsReady ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={trends}><defs><linearGradient id="spend" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#4ed5cf" stopOpacity=".52"/><stop offset="1" stopColor="#4ed5cf" stopOpacity="0"/></linearGradient></defs><XAxis dataKey="month" tickLine={false} axisLine={false}/><YAxis hide/><Tooltip formatter={(v: number) => currency(v)} contentStyle={{background:"#101c39",border:"1px solid #2a3b61",borderRadius:12}}/><Area dataKey="spend" type="monotone" stroke="#65e0d7" strokeWidth={3} fill="url(#spend)"/></AreaChart></ResponsiveContainer> : <div className="chart-placeholder" aria-label="Loading monthly spending chart"/>}</div></div>
