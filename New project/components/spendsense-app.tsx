...
export function CashFlowApp() {
  ...
  const currentMonth = liveTransactions.filter(t => t.date.startsWith(latestMonth) && t.kind === "debit");
  const currentMonthLabel = latestMonth ? new Intl.DateTimeFormat("en-IN", { month: "long" }).format(new Date(`${latestMonth}-01`)) : "Current month";
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Primary navigation">
        <button className={view === "dashboard" ? "nav-item active" : "nav-item"} onClick={() => setView("dashboard")}><Zap size={18}/> Overview</button>
        <button className={view === "subscriptions" ? "nav-item active" : "nav-item"} onClick={() => setView("subscriptions")}><CreditCard size={18}/> Subscription intelligence</button>
        <button className="nav-item" onClick={() => setSelected(anomalies[0])}><AlertTriangle size={18}/> Anomaly watch <em>{anomalies.length}</em></button>
      </nav>
      <div className="privacy-card"><ShieldCheck size={20}/><div><strong>Private by design</strong><p>Your demo data stays local.</p></div></div>
      <div className="persona"><span className="avatar">MS</span><div><strong>Marmik S.</strong><small>Young professional</small></div><ChevronRight size={16}/></div>
    </aside>
    <section className="content">
      <header className="topbar">
        <button className="menu-btn" aria-label="Open navigation"><Menu size={20}/></button>
        ...
        <div className="header-actions"><label className="month-select"><CalendarDays size={16}/><select defaultValue={currentMonthLabel} aria-label="Select month"><option>{currentMonthLabel}</option><option>Previous month</option></select></label><button className="entry-btn" onClick={() => setEntryOpen(true)}><Plus size={16}/> Add subscription</button><Link href="/login" className={`data-status ${dataSource === "live" ? "live" : ""}`}><span className="status-dot"/>{dataSource === "live" ? "Live data" : sessionEmail ? "Connected" : "Login"}</Link><button className="icon-btn" aria-label="Notifications"><Bell size={18}/><i/></button><button onClick={runScan} disabled={scanning} className="scan-btn"><ScanLine size={17}/>{scanning ? "Scanning…" : "Run AI Scan"}</button></div>
      </header>
      <div className="scenario-row"><span>Demo scenario</span>{scenarios.map(option => <button key={option} onClick={() => setScenario(option)} className={scenario === option ? "scenario selected" : "scenario"}>{option}</button>)}<span className="scenario-copy">{scenario === "Shared apartment" ? "Rent split is protected as a new shared commitment." : scenario === "Power subscriber" ? "More overlap signals surfaced for a subscription-heavy life." : "Balanced spending patterns with realistic drift."}</span></div>
      {scanning ? <ScanProgress stage={scanStage}/> : view === "dashboard" ? <Dashboard dataset={liveTransactions} currentMonthLabel={currentMonthLabel} subscriptions={visibleSubscriptions} anomalies={anomalies} monthlySpend={monthlySpend} potentialSavings={potentialSavings} chartsReady={chartsReady} onSelect={setSelected} onNavigate={() => setView("subscriptions")}/> : <Subscriptions subscriptions={visibleSubscriptions} onSelect={setSelected} onKeep={keep} classifications={classifications} setClassifications={setClassifications}/>}
    </section>
...
function Dashboard({ dataset, currentMonthLabel, subscriptions, anomalies, monthlySpend, potentialSavings, chartsReady, onSelect, onNavigate }: { dataset: Transaction[]; currentMonthLabel: string; subscriptions: Subscription[]; anomalies: Anomaly[]; monthlySpend: number; potentialSavings: number; chartsReady: boolean; onSelect: (item: Subscription | Anomaly) => void; onNavigate: () => void }) {
  const monthKeys = [...new Set(dataset.filter(t=>t.kind === "debit").map(t=>t.date.slice(0, 7)))].sort().slice(-10);
  const trends = monthKeys.map(month => ({ month: new Intl.DateTimeFormat("en-IN", { month: "short" }).format(new Date(`${month}-01`)), spend: dataset.filter(t=>t.kind === "debit" && t.date.startsWith(month)).reduce((sum,t)=>sum+t.amount,0) }));
  const latestMonth = monthKeys.at(-1) ?? "";
  const categoryData = Object.entries(dataset.filter(t=>t.date.startsWith(latestMonth) && t.kind === "debit").reduce<Record<string, number>>((a,t)=>{a[t.category]=(a[t.category]||0)+t.amount;return a;},{})).map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value);
  const recent = [...dataset].filter(t=>t.date.startsWith(latestMonth) && t.kind === "debit").sort((a,b)=>b.date.localeCompare(a.date)).slice(0,5);
  return <div className="dashboard-page">
    <section className="metric-grid">
      <Metric icon={<ArrowUpRight/>} label={`${currentMonthLabel} spend`} value={currency(monthlySpend)} trend="Live transaction total" tone="good"/>
      <Metric icon={<DollarSign/>} label="Potential monthly savings" value={currency(potentialSavings)} trend="2 subscriptions to review" tone="risk"/>
      <Metric icon={<CreditCard/>} label="Recurring commitments" value={String(subscriptions.length)} trend="1 new commitment learning" tone="learning"/>
      <Metric icon={<AlertTriangle/>} label="Spending anomalies" value={String(anomalies.length)} trend="Needs a quick look" tone="risk"/>
    </section>
    <section className="dashboard-grid"><div className="panel trend-panel"><div className="panel-head"><div><p className="eyebrow">SPENDING RHYTHM</p><h2>Monthly outflow</h2></div><Tone tone="good">−₹3.6k vs Jul</Tone></div><div className="chart">{chartsReady ? <ResponsiveContainer width="100%" height="100%"><AreaChart data={trends}><defs><linearGradient id="spend" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#4ed5cf" stopOpacity=".52"/><stop offset="1" stopColor="#4ed5cf" stopOpacity="0"/></linearGradient></defs><XAxis dataKey="month" tickLine={false} axisLine={false}/><YAxis hide/><Tooltip formatter={(v: number) => currency(v)} contentStyle={{background:"#101c39",border:"1px solid #2a3b61",borderRadius:12}}/><Area dataKey="spend" type="monotone" stroke="#65e0d7" strokeWidth={3} fill="url(#spend)"/></AreaChart></ResponsiveContainer> : <div className="chart-placeholder" aria-label="Loading monthly spending chart"/>}</div></div>
      ...
      <div className="panel recent-panel"><div className="panel-head"><div><p className="eyebrow">LIVE FEED</p><h2>Recent transactions</h2></div><button className="text-link">View all</button></div>{recent.map(t=><TransactionRow key={t.id} transaction={t}/>)}</div>
